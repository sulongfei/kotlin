...
class Fir2IrVisitor(
    ...
    private fun convertToImplicitCastExpression(
        ...
        if (castTypeRef is FirResolvedTypeRef && originalTypeRef is FirResolvedTypeRef) {
            ...
            val originalType = originalTypeRef.type
            if (originalType == castType) {
                return value
            }
            if (castType is ConeIntersectionType) {
                var targetType: ConeKotlinType? = null
                for (possibleType in castType.intersectedTypes) {
                    val scope = possibleType.scope(session, components.scopeSession) ?: continue
                    when (referencedSymbol) {
                        is FirPropertySymbol -> scope.processPropertiesByName(calleeReference.name) {
                            if (targetType == null && it == referencedSymbol) {
                                targetType = possibleType
                            }
                        }
                        is FirFunctionSymbol -> scope.processFunctionsByName(calleeReference.name) {
                            if (targetType == null && it == referencedSymbol) {
                                targetType = possibleType
                            }
                        }
